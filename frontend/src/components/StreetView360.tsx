...
import styles from "./StreetView360.module.css";
...
const AUTH_FAILURE_EVENT = "temporal-maps-auth-failure";
...
function isMapsReady(): boolean {
  return Boolean(window.google?.maps?.StreetViewService && window.google.maps.geometry?.spherical);
}
...
function loadGoogleMaps(): Promise<void> {
  ...
  mapsLoadPromise = new Promise<void>((resolve, reject) => {
    ...
    script.id = SCRIPT_ID;
    script.async = true;
    script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(apiKey)}&loading=async&libraries=streetView,geometry&callback=${CALLBACK_NAME}`;
    script.onerror = () => {
      cleanupFailedScript();
          ...
          <div className={styles.overlay}>
            ...
            >
              <header className={styles.dialogHeader}>
                ...
                <button ref={closeButtonRef} className={styles.close} onClick={close} type="button">
                  Close
                </button>
              </header>
              <div className={styles.viewport}>
                ...
                {viewState === "empty" ? <p className={styles.message}>{EMPTY_COPY}</p> : null}
                {viewState === "error" ? (
                  <p className={styles.error} role="alert">
                    {errorMessage}
                  </p>
                ) : null}
                {viewState === "ready" ? <div className={styles.panorama} ref={panoramaRef} /> : null}
              </div>
              {showCaveat ? (
                <p className={styles.caveat}>This view may not be precisely on the property.</p>
              ) : null}
            </div>
          </div>,
          document.body,
