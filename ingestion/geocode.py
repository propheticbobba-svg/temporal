...
class PhotonFeature(BaseModel):
    ...
    def _label(self) -> str:
        ...
        parts = [part for part in (street or props.name, props.city, props.state, props.postcode) if part]
...
class GeocodeIngester(BaseIngester):
    ...
    async def _fetch_matches(self, location: LocationInput) -> list[CensusAddressMatch]:
        try:
            ...
            logger.warning("Census geocoder failed for %s: %s; trying fallbacks", location.address, exc)
...
def compact_geocode_query(address: str) -> str:
    ...
    street = next((part for part in parts if re.search(r"\d", part)), parts[0] if parts else stripped)
    zip_part = next((part for part in reversed(parts) if re.search(r"\b\d{5}(?:-\d{4})?\b", part)), None)
